...
def zip_audio_files(
    ...
    """
    Zip the audio files of songs and return the zip as a binary object.
    
    Parameters:
    - songs: A list of songs to be zipped.
    ...
    with zipfile.ZipFile(buf, "w") as audio_zip:
        ...
        for i, item in enumerate(items):
            if isinstance(songs, dict):
                ...
                data = songs[item]
                safe_filename = format_safe_filename(item)
            else:
                if stqdm:
                ...
                item.download_audio()
                data = item._audio
                safe_filename = format_safe_filename(item.filename)
            try:
